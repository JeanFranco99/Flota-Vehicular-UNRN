/**
 * =================================================================
 * VERSIÓN 1: ESTÁTICA (CON ID HARDCODEADO)
 * =================================================================
 */
...
function obtenerMantenimientos() {
  const ss = SpreadsheetApp.getActiveSpreadsheet(); // ERROR PREVIO: Solo funciona atado a un sheet
  ...
  const mantenimientos = datos.map(fila => {
    ...
    cabecera.forEach((nombreColumna, i) => {
      obj[nombreColumna.trim().replace(/\s/g, '')] = fila[i];
    });
    return obj;
  });
...
function aprobarSolicitud(idSolicitud, patenteAsignada, fechaSalida, fechaRegreso, controlData = null) {
  ...
  try {
    if (controlData && controlData.items && controlData.items.length > 0) {
      try {
        registrarControl({ idSolicitud: idSolicitud, tipo: 'inicial', items: controlData.items, kilometros: controlData.kilometros || '', observaciones: controlData.observaciones || '' });
      } catch (controlError) { Logger.log('Error al registrar control inicial: ' + controlError.message); }
    }
    ...
    if (solicitudActual.estado.toLowerCase() !== 'pendiente') throw new Error(`No se puede aprobar esta solicitud porque su estado actual es "${solicitudActual.estado}".`);
    ...
    for (let i = 1; i < datosVehiculos.length; i++) {
      if (String(datosVehiculos[i][1]).toLowerCase() === String(patenteAsignada).toLowerCase()) {
        ...
        if (estadoVehiculo !== 'disponible') throw new Error(`El vehículo ${patenteAsignada} no se puede asignar porque su estado es "${estadoVehiculo}".`);
    ...
    let solicitudFechaSalida = null;
    let solicitudFechaRegreso = null;
    ...
    if (solicitudFechaSalida && solicitudFechaRegreso) {
      ...
      if (!fechaSalida || !fechaRegreso || fechaSalida.trim() === '' || fechaRegreso.trim() === '') throw new Error('Las fechas de salida o regreso no son válidas.');
      fechaSalidaValidada = parsearFecha(fechaSalida);
      fechaRegresoValidada = parsearFecha(fechaRegreso);
    }
    
    if (isNaN(fechaSalidaValidada.getTime()) || isNaN(fechaRegresoValidada.getTime())) throw new Error('Las fechas de salida o regreso no son válidas.');
    ...
    try { MailApp.sendEmail(emailSolicitante, asunto, "", {htmlBody: cuerpo}); } catch (e) {}
  ...
  if (hojaMantenimientos.getLastRow() >= 2) {
      ...
      for (const mant of mantenimientosProgramados) {
          ...
          if (fechaSalida.getTime() < finMantenimiento.getTime() && fechaRegreso.getTime() > inicioMantenimiento.getTime()) {
              throw new Error(`Conflicto de mantenimiento. El vehículo ${patente} tiene un mantenimiento programado.`);
          }
      }
  }
  return true;
...
function obtenerMantenimientosActivosConFechas() {
  ...
  return mantenimientosActivos.map(row => ({ id: row[0], patente: row[1], tipo: row[2], detalle: row[3], fechaInicio: row[6], fechaInicioFormateada: formatDateTime(row[6]) }));
}
...
function verificarDisponibilidadEspecifica(patente, fechaSalida, fechaRegreso) {
  ...
  try {
    ...
    return { disponible: false, mensaje: error.message, conflictos: [{ tipo: error.message.includes('mantenimiento') ? 'mantenimiento' : 'solicitud', detalle: error.message }] };
  }
}
...
function listarVehiculos() {
  ...
  datos.forEach((row, index) => {
    ...
    try {
      vehiculos.push({ id: row[0], patente: row[1], marca: row[2], modelo: row[3], anio: row[4], capacidad: row[5], estado: row[6], observaciones: row[7], combustible: row[8], velocidadMaxima: row[9], fechaVtoRto: formatDateTime(row[10]), fechaVtoSeguro: formatDateTime(row[11]) });
    } catch (e) { Logger.log(`Error al procesar vehículo N° ${index + 2}: ${e.message}`); }
  });
  return vehiculos;
}
...
function listarMantenimientosActivos() {
    ...
    return datos.filter(row => (row[8] || '').toLowerCase() === 'en curso')
        .map(row => ({ id: row[0], patente: row[1], tipo: row[2], detalle: row[3], costo: row[4], responsable: row[5], fechaInicio: formatDateTime(row[6]) }));
}
...
function listarMantenimientosFinalizados() {
    ...
    return datos.filter(row => (row[8] || '').toLowerCase() === 'finalizado')
        .map(row => ({ id: row[0], patente: row[1], tipo: row[2], detalle: row[3], costo: row[4], responsable: row[5], fechaInicio: formatDateTime(row[6]), fechaFin: formatDateTime(row[7]) }));
}
    ...
    try {
      ...
      for (let i = 1; i < datos.length; i++) {
          if (datos[i][0] == idSolicitud) {
              ...
              if (estadoActual !== 'pendiente') throw new Error(`Estado actual: "${estadoActual}".`);
              ...
              MailApp.sendEmail(emailSolicitante, "Solicitud Rechazada", "", {htmlBody: `Motivo: ${motivo}`});
